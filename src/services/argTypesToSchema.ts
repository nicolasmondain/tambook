 ...
 */
function shouldSkipProp(argType: ArgType, propName: string): boolean {
  // Never skip 'children' - we handle it specially for nesting support
  ...
  // Skip React-specific types that can't be generated by AI (except children)
  if (
    typeName.includes('reactnode') ||
    typeName.includes('element') ||
    typeName.includes('jsx') ||
...
/**
 * Check if a prop is a children/ReactNode type that should support nesting
 */
function isChildrenProp(propName: string): boolean {
  return propName === 'children';
}

/**
 * Create a simple JSON Schema for children that supports nesting
 * Uses a permissive schema - the prompt guides the AI on how to use it
 */
function createChildrenSchema(): JSONSchema7 {
  return {
    description: 'Nested content: string for text, or object { componentName: string, props: object } for nested component, or array of these',
  };
}
...
export function argTypesToJsonSchema(
  ...
  for (const [propName, argType] of Object.entries(argTypes)) {
    // Skip non-serializable props (except children)
    if (shouldSkipProp(argType, propName)) {
      continue;
    }

    // Handle children prop specially for nesting support
    if (isChildrenProp(propName)) {
      schema.properties![propName] = createChildrenSchema();
      continue;
    }
